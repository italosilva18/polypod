...
func NewService(client *Client, knowledge KnowledgeSearcher, ag *agent.Agent, logger *slog.Logger) *Service {
	if ag == nil {
...
func (s *Service) Answer(ctx context.Context, req AnswerRequest) (*AnswerResponse, error) {
	...
	systemPrompt := BuildSystemPrompt(s.agent.Persona, knowledgeCtx)
